...
Then launch the Super FP8 Reasoner in target-only mode with video-token pruning
disabled. This one-GPU configuration requires compute capability 8.9 or newer,
at least 67 GiB
...
The command pins Super FP8 target-only and disables video-token pruning so the
